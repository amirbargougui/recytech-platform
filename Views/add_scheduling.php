...
if(isset($_POST['schedule_booking'])){
    $id = create_unique_id();
    $item_type_id = $_GET['id'];
 
    $collectionDate = $_POST['date'];
    $collectionDate = filter_var($collectionDate, FILTER_SANITIZE_STRING);
 
    $collectionTime = $_POST['time'];
    $collectionTime = filter_var($collectionTime, FILTER_SANITIZE_STRING);
 
    $location = $_POST['location'];
    $location = filter_var($location, FILTER_SANITIZE_STRING);

    // Check if the item_type_id exists in the item_types table
    $check_item_type = $conn->prepare("SELECT id FROM item_types WHERE id = ?");
    $check_item_type->execute([$item_type_id]);

    if($check_item_type->rowCount() > 0) {
        // Insert the schedule record
        $add_schedule = $conn->prepare("INSERT INTO `schedule`(id, item_type_id, collection_date, collection_time, location) VALUES(?,?,?,?,?)");
        $add_schedule->execute([$id, $item_type_id, $collectionDate, $collectionTime, $location]); 

        $success_msg[] = 'Schedule added!';
    } else {
        $error_msg[] = 'Invalid item type ID';
    }
    ...
    $get_item = $conn->prepare("SELECT * FROM `item_types` where id=? ");
    $get_item->execute([$item_type_id ]);
    $item_types = $get_item->fetch(); // fetch() instead of fetchAll() since we only want one row

    // construction du message
$message = "<html><body><h2 style='color: #3f885c;'>Thank you for scheduling your collection with RecytTech</h2>";
$message .= "<h1>Your booking details :</h1>";
$message .= "<p><strong>Name:</strong> " . $item_types['name']. "</p>";
$message .= "<p><strong>Description:</strong> " . $item_types['description']. "</p>";
$message .= "<p><strong>Date:</strong> " . $collectionDate . "</p>";
$message .= "<p><strong>Time:</strong> " . $collectionTime . "</p>";
...
$message .= "</body></html>";
    ...
    // envoi du mail
    $to = $email;
    $subject = "Collection Schedule Reservation";
    $headers = "From: nessimayadi4@gmail.com" . "\r\n" .
               "Reply-To: nessimayadi4@gmail.com" . "\r\n" .
    ...
    header('Location: collection-schedule.php');
    exit;
}
?>
...
<html lang="en" class="no-js">
    ...
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.4.1/css/all.css" integrity="sha384-5sAR7xN1Nv6T6+dT2mhtzEpVJvfS3NScPQTrOxhwjIuvcA67KV2R5Jz6kr4abQsz" crossorigin="anonymous">
        <!-- CSS
        ================================================== -->
        <link rel="styleshheet" href="css/vendor.css">
...
<h2 class="form-title">Reserve your collection schedule</h2>
<form method="POST" class="register-form" onsubmit="return check()">


<div class="form-group" >
<label for="date" id="icon"><i class="fas fa-calendar-alt"></i></label>
<input type="date"  name="date" id="date"    class="no-outline">
<span id="error_date" style="display:none;"></span>

</div>
<div class="form-group">
<label for="time" id="icon"><i class="fas fa-clock"></i></label>
<input type="time" name="time" id="time"  class="no-outline"/>
<span id="error_time" style="display:none;"></span>

</div>

<div class="form-group">
<label for="location" id="icon"><i class="fas fa-map-marked-alt"></i></label>
<input type="text" name="location" id="location" placeholder="Location"  class="no-outline"/>
<span id="error_location" style="display:none;"></span>
...
<div class="form-group">
    ...
    <input type="email" name="email" id="email" placeholder="Email" class="no-outline"/>
    <span id="error_email" style="display:none;"></span>
</div>
...
<div class="form-group">
    <div class="form-group">
        <label class="label-agree-term">
            ...
            <span class="label-text">I agree all statements 
            </span>
          </label>
    </div>
</div>
<div class="form-group">
    <button type="submit" name="schedule_booking" id="schedule_booking">Reserve</button>
    </a>
  </div>
</form>
</div>
...
<div class="calendar">
 <figure><img src="images/calendrier-booking1.jpg" alt="sing up image"></figure>
</div>
</div>
</div>
</section>
